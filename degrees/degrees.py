    ...
    while True:
        ...
        if node.state == goal.state:
            ...
            while node.parent is not None:
                ...
                node = node.parent
                result.reverse()
            return result #Creo que aquí hay que añadir un reverse
        
        #Mark node as explored
        explored.append(node.state)

        #Add neighbors to frontier
        for id_movie, id_person in neighbors_for_person(node.state):
            if not frontier.contains_state(id_person) and id_person not in explored:
                frontier.add(Node(state = id_person, parent=node, action= id_movie))
